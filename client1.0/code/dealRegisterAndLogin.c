...
void disableEcho(int fd, struct termios *old_termios) {
    ...
    tcgetattr(fd, old_termios); // 获取当前终端的设置，并保存到“old_termios"结构体中
    new_termios = *old_termios; // 将当前终端设置复制到”new_termios"结构体中
    new_termios.c_lflag &= ~(ECHO); // 关闭回显功能，清除”ECHO“标志位
    tcsetattr(fd, TCSANOW, &new_termios); // 立即更新终端设置，禁用回显功能
}
// 用于恢复终端回显的函数
void restoreEcho(int fd, struct termios *old_termios) {
    tcsetattr(fd, TCSANOW, old_termios); // 恢复之前保存的终端设置，重新启用回显功能
}
